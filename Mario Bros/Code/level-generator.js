...
Mario.LevelGenerator.prototype = {
  ...
  BuildZone: function (level, x, maxLength) {
    var t = (Math.random() * this.TotalOdds) | 0,
      type = 0,
      i = 0;
    for (i = 0; i < this.Odds.length; i++) {
      if (this.Odds[i] <= t) {
  ...
  BuildJump: function (level, xo, maxLength) {
    var js = ((Math.random() * 4) | 0) + 2,
      jl = ((Math.random() * 2) | 0) + 2,
      length = js * 2 + jl,
      x = 0,
      y = 0,
      hasStairs = ((Math.random() * 3) | 0) === 0,
      floor = this.Height - 1 - ((Math.random() * 4) | 0);
  ...
  BuildHillStraight: function (level, xo, maxLength) {
    var length = ((Math.random() * 10) | 0) + 10,
      floor = (this.Height - 1 - Math.random() * 4) | 0,
      x = 0,
      y = 0,
      h = floor,
  ...
  BuildTubes: function (level, xo, maxLength) {
    var length = ((Math.random() * 10) | 0) + 5,
      floor = (this.Height - 1 - Math.random() * 4) | 0,
      xTube = (xo + 1 + Math.random() * 4) | 0,
    ...
    for (x = xo; x < xo + length; x++) {
      ...
      if (x === xTube && ((Math.random() * 11) | 0) < this.Difficulty + 1) {
        level.SetSpriteTemplate(
          x,
          tubeHeight,
          new Mario.SpriteTemplate(Mario.Enemy.Flower, false)
        );
      }
