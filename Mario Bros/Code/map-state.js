// The map of the different levels
Mario.MapTile = {
  Grass: 0,
  Water: 1,
  Level: 2,
...
Mario.MapState.prototype.GenerateLevel = function () {
  ...
  var connection = true;
  while (connection) {
    connection = this.FindConnection(width, height);
  }
  ...
  for (x = 0; x < width; x++) {
    for (y = 0; y < height; y++) {
      if (
        ...
        (x !== this.XFarthestCap || y !== this.YFarthestCap - 1)
      ) {
...
Mario.MapState.prototype.RenderStatic = function () {
  var x = 0,
    ...
    xx = 0,
    yy = 0,
    image = Engine.Resources.Images["worldMap"],
    type = 0;
...
Mario.MapState.prototype.TryWalking = function (xd, yd) {
  ...
  if (
    this.Level[xt][yt] === Mario.MapTile.Road ||
    this.Level[xt][yt] === Mario.MapTile.Level
  ) {
...
Mario.MapState.prototype.CheckForChange = function (context) {
  ...
  if (this.EnterLevel) {
    context.ChangeState(
      new Mario.LevelState(this.LevelDifficulty, this.LevelType)
    );
  }
};
