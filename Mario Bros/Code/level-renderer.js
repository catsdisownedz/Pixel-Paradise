  ...
  for (x = (camera.X / 16) | 0; (x <= (camera.X + this.Width) / 16) | 0; x++) {
    for (
      ...
      if ((Mario.Tile.Behaviors[b & 0xff] & Mario.Tile.Animated) > 0) {
        ...
        frame =
          this.Background[(((b % 16) / 4) | 0) * 4 + animTime][(b / 16) | 0];
        context.drawImage(
          Engine.Resources.Images["map"],
          frame.X,
          frame.Y,
...
Mario.LevelRenderer.prototype.DrawExit0 = function (context, camera, bar) {
  ...
  for (y = this.Level.ExitY - 8; y < this.Level.ExitY; y++) {
    ...
    context.drawImage(
      ...
      frame.Width,
      frame.Height,
      (this.Level.ExitX << 4) - camera.X - 16,
      (y << 4) - camera.Y,
      frame.Width,
      frame.Height
    );
